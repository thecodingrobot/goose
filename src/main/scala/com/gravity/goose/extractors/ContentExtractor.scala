...
trait ContentExtractor extends StrictLogging {
  ...
  def getTitle(article: Article): String = {
    ...
    try {
      ...
      logger.trace(logPrefix + "Page title is: " + title)
  ...
  def calculateBestNodeBasedOnClustering(article: Article): Option[Element] = {
    logger.trace(logPrefix + "Starting to calculate TopNode")
    val doc = article.doc
    ...
    logger.trace(logPrefix + "About to inspect num of nodes with text: " + numberOfNodes)
    ...
    for (node <- nodesWithText) {
      ...
      logger.trace(logPrefix + "Location Boost Score: " + boostScore + " on interation: " + i + "' id='" + node.parent.id + "' class='" + node.parent.attr("class"))
    ...
    for (e <- parentNodes) {
      ...
      logger.trace(logPrefix + "ParentNode: score='" + e.attr("gravityScore") + "' nodeCount='" + e.attr("gravityNodes") + "' id='" + e.id + "' class='" + e.attr("class") + "' ")
  ...
  def printTraceLog(topNode: Element): Unit = {
    try {
      if (topNode != null) {
        logger.trace(logPrefix + "Our TOPNODE: score='" + topNode.attr("gravityScore") + "' nodeCount='" + topNode.attr("gravityNodes") + "' id='" + topNode.id + "' class='" + topNode.attr("class") + "' ")
        val text = if (topNode.text.trim.length > 100) topNode.text.trim.substring(0, 100) + "..." else topNode.text.trim
        logger.trace(logPrefix + "Text - " + text)
      }
    } catch {
      case e: NullPointerException => logger.warn(s"printTraceLog ${e.getMessage}", e)
    }
  }
      ...
      currentNode => {
        if (currentNode.tagName == para) {
          if (stepsAway >= maxStepsAwayFromNode) {
            logger.trace(logPrefix + "Next paragraph is too far away, not boosting")
            return false
          }
          ...
          if (wordStats.getStopWordCount > minimumStopWordCount) {
            logger.trace(logPrefix + "We're gonna boost this node, seems contenty")
            return true
          }
          stepsAway += 1
        }
  ...
  private def isHighLinkDensity(e: Element): Boolean = {
    ...
    logger.trace(logPrefix + "Calulated link density score as: " + score + " for node: " + getShortText(e.text, 50))
  ...
  def extractVideos(node: Element): List[Element] = {
    ...
    val vimdeoStr = "vimeo"
    try {
      node.parent.getElementsByTag("embed").asScala.foreach(candidates.add)
      node.parent.getElementsByTag("object").asScala.foreach(candidates.add)

      logger.trace(logPrefix + "extractVideos: Starting to extract videos. Found: " + candidates.size)
        ...
        for (a <- attrs.asScala) {
          try {
            if ((a.getValue.contains(youtubeStr) || a.getValue.contains(vimdeoStr)) && (a.getKey == "src")) {
              logger.trace(logPrefix + "This page has a video!: " + a.getValue)
              goodMovies += el
            }
          }
          catch {
            case e: Exception =>
              logger.info(logPrefix + "Error extracting movies: " + e.toString)
          }
        }
      }
    }
    ...
    logger.trace(logPrefix + "extractVideos:  done looking videos")
    goodMovies.toList
  }
  ...
  def isTableTagAndNoParagraphsExist(e: Element): Boolean = {
    ...
    if (subParagraphs2.size == 0 && !(e.tagName == "td")) {
      logger.trace("Removing node because it doesn't have any paragraphs")
      true
    } else {
      false
    }
  ...
  def postExtractionCleanup(targetNode: Element): Element = {
    ...
    logger.trace(logPrefix + "Starting cleanup Node")
    val node = addSiblings(targetNode)
    for {
      e <- node.children.asScala
      if e.tagName != "p"
    } {
      logger.trace(logPrefix + "CLEANUP  NODE: " + e.id + " class: " + e.attr("class"))
      if (isHighLinkDensity(e) || isTableTagAndNoParagraphsExist(e) || !isNodeScoreThreshholdMet(node, e)) {
        try {
          e.remove()
        } catch {
          case ex: IllegalArgumentException => logger.trace("Cannot remove node: " + ex.toString)
        }
      }
    }
    node
  ...
  def isNodeScoreThreshholdMet(node: Element, e: Element): Boolean = {
    ...
    logger.trace(logPrefix + "topNodeScore: " + topNodeScore + " currentNodeScore: " + currentNodeScore + " threshold: " + thresholdScore)

    if ((currentNodeScore < thresholdScore) && e.tagName != "td") {
      logger.trace(logPrefix + "Removing node due to low threshold score")
      false
    } else {
      logger.trace(logPrefix + "Not removing TD node")
      true
    }
  }
    ...
    if (currentSibling.tagName == "p" && currentSibling.text.length() > 0) {
      ...
      if (potentialParagraphs.first == null) {
        ...
        Some((for {
          ...
          logger.trace(logPrefix + "This node looks like a good sibling, adding it")
  ...
  def walkSiblings[T](node: Element)(work: (Element) => T): Seq[T] = {
    ...
    while (currentSibling != null) {
      ...
      logger.trace(logPrefix + "SIBLINGCHECK: " + debugNode(currentSibling))
  ...
  private def addSiblings(topNode: Element): Element = {
    ...
    logger.trace(logPrefix + "Starting to add siblings")
